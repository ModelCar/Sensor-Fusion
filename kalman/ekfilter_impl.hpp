...
namespace Kalman {
  ...
  template<typename T, K_UINT_32 BEG, bool OQ, bool OVR, bool DBG>
  void EKFilter<T, BEG, OQ, OVR, DBG>::step(Vector& u_, Vector& z_sensor, Vector& z_depth) {
    timeUpdateStep(u_);
    measureUpdateStep(z_sensor, 1);
    measureUpdateStep(z_depth, 2);
  }

  template<typename T, K_UINT_32 BEG, bool OQ, bool OVR, bool DBG>
  void EKFilter<T, BEG, OQ, OVR, DBG>::step(Vector& u_, Vector& z_depth) {
    timeUpdateStep(u_);
    measureUpdateStep(z_depth, 2);
  }
  ...
  template<typename T, K_UINT_32 BEG, bool OQ, bool OVR, bool DBG>
  void EKFilter<T, BEG, OQ, OVR, DBG>::measureUpdateStep(Vector& zz, K_UINT_32 step) {
    ...
    if (step == 1)
    {
	z_(1) = zz(1)/2*sqrt(temp_A*temp_A-temp_B*temp_B)/abs(temp_A);
    }
    else if (step == 2)
    {
	z_(1) = zz(1);
  ...
  template<typename T, K_UINT_32 BEG, bool OQ, bool OVR, bool DBG>
  void EKFilter<T, BEG, OQ, OVR, DBG>::makeV1(const Vector& z) {
    NoModification();
  }

  template<typename T, K_UINT_32 BEG, bool OQ, bool OVR, bool DBG>
  void EKFilter<T, BEG, OQ, OVR, DBG>::makeV2() {
    NoModification();
  }
  ...
  template<typename T, K_UINT_32 BEG, bool OQ, bool OVR, bool DBG>
  void EKFilter<T, BEG, OQ, OVR, DBG>::makeVImpl(const Vector& z_, K_UINT_32 step) {
    modified_ = true;
    if (step == 1)
    {
	makeV1(z_);
