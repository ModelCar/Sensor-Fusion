...
namespace Kalman {
  ...
  template<typename T, K_UINT_32 BEG, bool OQ = true, 
  ...
  public:
    ...
    void step(Vector& u_, Vector& z_sensor, Vector& z_depth);
    void step(Vector& u_, Vector& z_depth);
    ...
    //! state.
    //! \param z_ Measurement vector. Will \b not be destroyed. Can be empty.
    void measureUpdateStep(Vector& zz, K_UINT_32 step);
    ...
    //! that a new prediction (and many other functions) will invalidateſ
  ...
  protected:
    ...
    //! it is not necessary to initialize non-diagonal elements with anything
    //! meaningful.
    virtual void makeV1(const Vector& z);
    virtual void makeV2();
    ...
    double rc, temp_A, temp_B, rcd_1, rcd_2, rcd_3;
  ...
  private:
    ...
    void makeVImpl(const Vector& z, K_UINT_32 step);
